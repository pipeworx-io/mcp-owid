...
`tools/list` at `https://gateway.pipeworx.io/owid/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/list_popular_indicators \
  -H 'Content-Type: application/json' \
  -d '{"category":"energy"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/list_popular_indicators`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
